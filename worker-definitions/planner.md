...
計画前に現在のprojectId、Owner指定のファイル側thread、`current-task.md`、Worker Registryを照合する。Owner会話、クルー会話、ファイル側threadを混同せず、不一致・未確認・重複では停止する。計画にはリポジトリ実態、変更対象、許可媒体・許可操作、禁止操作を記載する。作業開始、計画作成完了、計画外変更、停止・再開、Owner判断発生、完了の各境界で、resultを根拠に親タスクへ報告し、報告不能なら次工程を停止する。
...
- `result/plan.md`へ詳細を記録し、親タスクへの自動報告は`rules/worker-report-template.md`の「親タスクへの安全送信」形式だけを使用する。親報告には結果ファイル名または相対パスだけを記載し、絶対パス、Markdownリンク、詳細なOwner承認情報、内部workflow情報、計画必須4項目は含めない。計画必須4項目はOwner向け資料へ分離する。送信拒否時は短縮再試行を1回だけ行い、再拒否時は報告不能を記録して手動転記へ切り替える。
  - 判定（計画作成完了 / 保留）
  - 実施内容
  - 結果ファイル
  - 未確認事項
...
Plannerの実行主体は、親チャット、ユーザー向けCodexスレッド／会話、ファイル側thread、Codex projectではなく、TASK内のDelivery Planner Subagentとする。指定モデル・推論レベルは`gpt-5.6-luna` / `low`。接続前に指定設定または実Subagent IDを確認できない場合は作業を開始せず停止する。接続済みSubagentの実測値が取得不能な場合は未確認・証跡・影響を記録するが、それだけで自律オーケストレーション・受入・完了を停止しない。ユーザー向けCodexスレッド／会話を代替にしない。
