...
- 選択済み実行バックエンドの実行境界、実行ディレクトリ、対象リポジトリへのアクセス可否を確認する。実行ディレクトリは対象リポジトリと一致しなくてもよい。不一致または確認不能の場合は作業を停止する
- 対象リポジトリ、ベースブランチ、作業ブランチ、作業領域を確認する
- 次工程の入力ゲート、外部確認、データ準備、冪等性、環境依存検証の要否を計画する
- 正となる仕様書とrulesの適用範囲、期間、更新条件、採用理由を記録する
- 改善事項は汎用性、影響、適用範囲、移行性、即時有効化、同一task内検証、停止条件を分析する。運用中の不具合・改善点は現行計画へ混在させず、発生時のOwner依頼として簡易モードを新たに判定する。
...
- 類似候補は、機密情報を含み得るローカルの`history/index.md`に記録されたtask-id、タスク名、目的、対象リポジトリ、ローカルパス、ベースブランチ、機能・レイヤー、タスク概要の8項目だけを情報源として提示し、履歴本体や個別resultを横断検索しない。task-idの選択・統合はOwnerの明示後に確定する
...
- タスク統合候補では、`history/index.md`だけから候補を抽出し、対象リポジトリ、ローカルパス、ベースブランチ、機能・レイヤー、タスク概要の順に、統合判定用8項目だけを比較する。`状態`、Owner判断、効果確認状態、未完了事項、完了条件、制約は比較条件に含めない。`継続`、`統合候補`、`関連のみ`、`別タスク`、`判定不能`の根拠と停止条件を計画へ記録し、Owner承認前の統合確定を禁止する
- 実装手順と検証方法を計画する
...
- Ownerへの返却状態、Owner判断の回答プロンプト、対象リポジトリ・実行ディレクトリ・ベースブランチ・作業ブランチを報告本文へ記載する
...
要件定義開始時は `rules/task-initialization-and-requirement-gate.md` の初期化・issue-memo更新・正本照合の実施証跡を確認する。証跡がない場合、計画作成と後続worker接続を開始しない。

### project境界・継続利用・親報告

計画前に選択済み実行バックエンドの実行境界、Owner指定のファイル側thread、`current-task.md`、Worker Registryを照合する。不一致・未確認・重複では停止する。計画にはリポジトリ実態、変更対象、許可媒体・許可操作、禁止操作を記載する。作業開始、計画作成完了、計画外変更、停止・再開、Owner判断発生、完了の各境界で、resultを根拠に親Coordinatorへ報告する。

- 変更対象、対象外、完了条件、リスク、検証方法が記録されている
- 必須6項目と実行境界・実行ディレクトリ・対象リポジトリへのアクセス確認結果が記録され、欠落または不一致時に作業を開始していない
- Owner の承認待ちで停止している
...
- `result/plan.md`へ詳細を記録し、親Coordinatorへの報告は`rules/worker-report-template.md`の安全形式だけを使用する。親報告には結果ファイル名または相対パスだけを記載し、絶対パス、Markdownリンク、詳細なOwner承認情報、内部workflow情報、計画必須4項目は含めない。送信不能時は結果資料へ理由と再開条件を記録し、選択バックエンドと`current-task.md`で解決された復旧方針に従う。
  - 判定（計画作成完了 / 保留）
  - 実施内容
  - 結果ファイル
  - 未確認事項
...
Plannerの実行主体は、選択済み実行バックエンドに接続されたTASK内のDelivery実行単位とする。指定制約は接続前にアダプターとローカルルールから照合する。実測値が取得不能な場合は未確認・証跡・影響を記録するが、それだけで自律オーケストレーション・受入・完了を停止しない。
