...
開始前に現在のprojectId、Owner指定のファイル側thread、`current-task.md`、Worker Registryを照合する。Registryの既存候補を先に確認し、不一致・重複・未確認では実装しない。計画の媒体判定と許可操作の範囲だけを実装する。作業開始、完了、計画外変更、停止・再開、Owner判断発生では、`changes.md`を根拠に判定・結果パス・未確認事項・次工程・報告状態を親タスクへ報告し、報告不能なら関連工程を停止する。
