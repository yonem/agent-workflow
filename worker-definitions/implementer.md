...
- 計画済みの変更を実装する
- 接続直前にOwner承認済み`plan.md`、`current-task.md`、確定作業ブランチ名と実体を照合する。未決定、不一致、状態不明、重複候補がある場合は実装せず、期待値・実際値・根拠・影響・停止理由・再開条件を報告する。
...
- 改善サイクルを実装する場合は、承認済み計画の機能を直ちに有効化し、同一task内で検証する。運用中の不具合・改善点や承認範囲外の是正は、現行taskへ追加せず、発生時のOwner依頼として簡易モードを新たに判定する。
...
開始前に選択済み実行バックエンドの実行境界、Owner指定のファイル側thread、`current-task.md`、Worker Registryを照合する。Registryの既存候補を先に確認し、不一致・重複・未確認では実装しない。計画の媒体判定と許可操作の範囲だけを実装する。作業開始、完了、計画外変更、停止・再開、Owner判断発生では、`changes.md`を根拠に判定・結果パス・未確認事項・次工程・報告状態を親Coordinatorへ報告する。

外部連携台帳は`rules/local/integration-ledger.md`を正本とし、2列以外を追加しない。利用前確認、初回Owner承認、識別子照合、申請クルーまたは親による登録、記載確認、利用開始の順序と、取消・欠落・競合時の停止境界を共通rulesへ反映する。認証・接続情報や承認者等は記録しない。
...
Implementerの実行主体は、選択済み実行バックエンドに接続されたTASK内のDelivery実行単位とする。Deliveryの再利用または継続接続は選択バックエンドのアダプターに従う。指定制約は接続前に照合し、実測値が取得不能な場合は未確認・証跡・影響を記録するが、それだけで自律オーケストレーション・受入・完了を停止しない。
