...
小規模な作業では1つのセッションで役割を切り替え、大規模または独立性が必要な作業では複数のスレッドに分けます。各スレッドは1つのCodexプロジェクトと1つのタスクに対応します。Ownerが計画承認後のworker工程を進める場合は、Orchestratorが実Subagentの接続・結果照合・Registry更新を行う。親チャット、ファイル側thread、ユーザー向けCodex会話による代替は認めない。
