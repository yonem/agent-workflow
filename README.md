...
小規模な作業では一つの実行単位で役割を切り替え、大規模または独立性が必要な作業では複数のスレッドに分けます。各スレッドは一つの選択済み実行バックエンドの実行境界と一つのタスクに対応します。Ownerが計画承認後のworker工程を進める場合は、Workflow Coordinatorが実行単位の接続・結果照合・Registry更新を行う。実行バックエンド固有の操作はアダプターに分離します。

## 目的

このリポジトリは、AIエージェントを開発プロセスへ組み込むための、実行バックエンド中立のルール・成果物・プロンプトを提供します。
...
IM・要件定義はOwnerと人間が直接行う一問一答であり、Coordinatorは介入しません。一問一答は判断回数や通知の最適化対象外です。人間がIMを確定しPlannerへの計画作成を明示指示した後、計画を承認した時点からだけCoordinatorが自律サイクルを開始し、Reviewer受入で終了します。Reviewer受入結果をOwnerが明示承認した場合だけ、Documenterの記録工程を開始します。

すべての依頼は、まず簡易モードで対象・安全性・影響範囲を判定します。低リスクで独立した変更は単一実行で完結します。完了時には適格理由または昇格理由を一文で示し、複数の簡易対応では対象・結果・理由をまとめて返します。Ownerがタスク外として即時反映を求め、影響範囲を直接参照先へ閉じられる共通ルール更新も、再読込・依存関係・冪等性の確認を伴う簡易対応として実施できます。task工程、外部操作、未確定要件などを含む場合は、確認済み事実だけを引き継いで標準IMへ自動昇格します。Ownerが`標準タスクで対応`または`計画から対応`と指定した場合も、最初から標準IMを開始します。複数の簡易対応候補を扱う場合だけ、簡易IMへ選択肢と結果を備忘録として残します。
...
選択済み実行バックエンドの観測値や実行境界が不一致になった場合は、作業を開始せず、結果資料・Registry・handoff eventを照合して復旧可否を判定します。復旧手順は `rules/thread-operation.md` の「同期失敗時の復旧」を参照してください。

Ownerがヘルスチェックを明示した場合、または作業中に実行単位へ接続できなかった場合は、`rules/worker-health-check.md` に従って選択バックエンドの識別子、親Coordinator境界、状態、指定制約・実測値、結果資料、重複、不一致、不足を読み取り確認します。接続失敗時も自動作成へ進まず、バックエンド固有の状態変更はアダプターとOwner判断に従います。

各スレッドは選択済み実行バックエンドの実行境界と1対1で対応させ、通常時は一つのactiveなtaskだけを扱います。復旧時の例外を含む分離・履歴・整合性の詳細は `rules/thread-operation.md` と `rules/workflow-consistency-check.md` に従います。

ファイル側threadの初期登録は資料保存領域だけを準備する非実行工程です。実行会話、worker、実行境界、task、識別子、history台帳は作成・接続・採番せず、要件定義開始後に別ゲートで扱います。作業開始、worker接続、履歴退避、復旧、task切替の前に、選択済みバックエンドの実行境界、Ownerが指定したファイル側thread、`current-task.md`の対象リポジトリ・実行ディレクトリを照合します。Owner会話は判断・承認、実行単位はworker実行、ファイル側threadは資料保存のための別単位です。不一致、未確認、複数候補では資料操作・接続・履歴操作を停止します。
...
Owner依頼
  ↓ 簡易モードの入口判定
適格な簡易対応 ──→ 単一実行・最小確認・短い結果応答
  │
  └─ 昇格条件または標準指定
       ↓
     標準IM → Planner → Owner計画承認 → 自律工程 → Reviewer受入
       → OwnerのDocumenter開始承認 → Documenter → Owner完了判断
```
...
人間向け資料と管理ファイルの共通形式、正本、責任、初期作成、復元、移行手順は `rules/human-facing-documentation.md` に従います。`owner-judge.md`、`task-progress.md`、`issue-memo.md`の形式や、サマリーと詳細の昇順・トレース規則も同ルールを参照します。
...
1. 変更依頼はそのまま伝える。簡易モードが入口判定を行い、適格なら直ちに対応する
2. 簡易対応の完了時は、判定理由と4項目の短い状態を確認する。複数対応では対象・結果・理由の一覧も受け取る
3. 複数の簡易対応候補がある場合は、簡易IMへ候補・実施順・結果を記録する
4. `標準タスクで対応`または`計画から対応`と指定した場合、または昇格条件に該当した場合は、確認済み事実の最小引継ぎを受けて標準IMを開始する
5. 標準IMを確定し、Plannerへの計画作成を指示する
6. 人間が計画・リスク・完了条件を承認する
7. Implementerに承認済みの範囲だけを実装させる
8. TesterとSecurity Operatorに、それぞれ実測ベースの検証と安全性確認を依頼する
9. 両方の完了報告を確認してからReviewerに独立レビューを依頼する
10. Reviewer受入後、Coordinatorは受入結果をOwnerへ返し、Ownerの明示承認がある場合だけDocumenterが受入根拠・変更・未確認事項を記録する
11. Documenterの通常task記録完了後、Ownerが完了を判断する
12. 運用中の不具合・改善点は、その発生時に新たな簡易モードの依頼として扱い、適格性に応じて即時対応または標準IMへ昇格する
13. 人間が採用、修正、中止、マージ、リリースを判断する
...
- Reviewer受入、Reviewer受入結果に対するOwnerのDocumenter開始承認、Documenter記録、Owner完了判断を完了境界とし、実運用後の効果確認、継続評価、再評価、新TASK起票、停止条件を分離して確認できる
...
`threads/<thread-name>/` は、選択済み実行バックエンドの実行境界と1対1で対応するタスク単位の作業領域です。スレッド名は用途に応じて自由に決定し、新しいスレッドは同じ構成で追加します。`rules/` 配下のMarkdownファイルは共通ルールとして扱い、適用範囲や優先順位は `rules/README.md`、スレッドの分離方法は `rules/thread-operation.md` を参照してください。

## 役割

`Owner`は人間が直接指示・承認を行う役割です。`Workflow Coordinator`はタスク進行を担い、実行単位の接続、正本照合、復旧監査、停止、承認要求を担当します。実行バックエンド固有の会話・通知・監視機構はアダプターに分離し、共通ルールでは両者の責務と証跡だけを定義します。
