...
- サマリーと詳細のIR-ID集合が一致しない、重複する、IDを比較できない場合はIRの更新を完了扱いにせず、Owner Agentが修正する。task本体は停止しない。
- `improvement-reminders.md`本文には運用ルール、責任分担、停止条件、変更履歴を記載せず、サマリーと詳細だけを記録する。運用ルールは本共通rulesを正本とする。
- IRの追加、更新、状態変更、継承、除外、リマインド判断はOwner Agentが担当する。
- Owner Agentは人間から指示を受ける起点Agentであり、人間と同一視しない。
...
- 対応済みIRは正本から削除し、IR履歴として保持しない。
- 現行IRがすべて削除された場合は`improvement-reminders.md`自体を削除する。次回の新規作成時はIR-001から採番する。
- IR-IDの採番は現行ファイルに存在するIRだけを対象とし、削除済みIRの番号を予約しない。
- IRの状態は`未対応`、`対応中`、`対応済み`、`延期`、`別タスク候補`、`Owner除外`で管理する。
- task-idは計画承認後にだけ採番する。対応未定のIRへtask-idを付与しない。
- 新規imへはOwnerが選択した未対応IRだけを継承し、未選択IRは継承しない。
- active taskは1つに限定する。

## IR固有の再実行補足

- 共通rules全体への適用は`rules/idempotency.md`に従う。
- IRについては、現行ファイルの存在、IR-ID集合、サマリー・詳細の一致、削除後の再採番を毎回確認する。

## リマインドと停止境界

- Documenterが通常のtask記録を完了した時点で、Owner Agentは必ずIR正本を確認し、対象IRを人間へリマインドする。Documenter自身はIRを管理・照合・通知しない。
- Owner AgentはDocumenter記録完了以外の必要な時点でも、追加リマインドを行える。
...
- IR判断が必要な場合だけ、Owner Agentが人間へ確認を求める。

## Documenter記録完了時のOwner Agentゲート

1. Documenterの通常task記録完了を親タスクが受領する。
2. Owner Agentが`improvement-reminders.md`の全IRを確認する。
...
- 実運用のIR確認・人間への提示・結果記録は、Documenterの通常task記録完了をOwner Agentが受領した後に限り、Owner Agentだけが実施する。Documenterおよび他workerは実運用ゲートを代行しない。
- Owner Agentは実運用ゲートごとに、Documenter記録完了の根拠、確認日時、IR正本の有無、確認したIR-ID集合、対象IR-ID・状態・内容、提示結果、IR不一致・未分類・取得失敗の有無、IR理由でtask本体を停止しない判定を`task-log.md`へ記録する。IR正本がない場合は、確認したIR-ID集合と対象IRを空集合、提示結果を0件として記録する。
