...
IM・要件定義はOwnerと人間が直接行う一問一答であり、Workflow Coordinatorの責務ではない。Coordinatorは、IMの質問・回答・判断・記録更新を代行、集約、最適化、再通知、event化してはならない。IMを確定してPlannerへの計画作成が明示指示されるまで、worker接続、handoff event、heartbeat、Worker Registry、自律サイクルを開始しない。`rules/quick-operation.md`の適格な簡易対応も、task工程ではないためCoordinator、worker、event、heartbeatを開始しない。

計画承認後からReviewer受入までがCoordinatorの自律工程である。判断最適化、重複抑止、修正依頼の集約、再試行制御はこの自律工程だけに適用し、IMの一問一答とReviewer受入後のDocumenter記録工程へ適用しない。Reviewer受入後は自律工程を終了し、Ownerの明示承認を待つ。
...
- Deliveryは同一task内でPlanner、Implementer、Documenterの論理責務を順次担当するため、Owner承認後のImplementer、およびReviewer受入結果をOwnerが明示承認した後のDocumenterへの切替は、閉じた同一Delivery Subagentをresumeして行える。別task、未確認の状態、または例外条件の不足を理由に新規Subagentを追加しない。
...
3. Reviewer受入後、AgentがDocumenter開始のOwner承認要求を起動し、自律工程を終了する。
4. Owner承認後にDocumenter記録工程へ切り替える。
...
Agentの受入要求起動、Reviewer受入、Documenter開始のOwner承認、Documenter記録は別イベントとして記録し、前イベントの成功を次イベントの発動条件とする。承認・受入の未応答は成功とみなさない。
