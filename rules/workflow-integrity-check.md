...
読み取り専用の自動化ゲートに共通する入力、判定値、出力形式、人の判断境界は`rules/read-only-automation-gates.md`を正本とする。本書は正本整合チェックの詳細な照合手順を定義し、入力資料の更新や是正を自動化しない。
...
履歴台帳のサマリー・詳細パートを照合する場合、hisi状態は`未着手`・`対応中`・`完了`の3値だけを許容する。`継続評価`等は履歴・IMP/IR内部の説明として扱い、hisi状態へ混入させない。計画承認後の実装・検証・記録の根拠がない行は推測で補正せず、6項目の確認報告とする。実運用観測や効果確認の未実施だけで現行taskを停止しない。
...
- 承認済み範囲の実装・検証・受入・記録が不足したまま次工程へ進めようとしている。IMP判定表や効果確認の未実施だけではtask本体を停止しない
