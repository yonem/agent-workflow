...
- Workflow Coordinatorは、対象taskを管理するCoordinator threadとする。worker実行単位は同threadに接続したDeliveryまたはReviewer Subagentとする。
...
- 本規約の追加だけでactive taskのない既存Coordinator、Subagent、heartbeatを作成・再開・有効化してはならない。
...
- 計画承認前、Reviewer受入後でDocumenter開始のOwner承認待ち、Documenter記録完了後、active taskなし、または同一原因の再試行が2回に達した後は`PAUSED`とする。
