...
- 用語集の初期作成・復元・移行先有効化は`rules/glossary.md`の手順に従い、`用語`・`標準的な意味`・`備考（取り違え防止）`の3列表、基準語4語、更新責任、参照先、停止条件を確認できるまで有効化完了としない。
...
`scope`はルールの主な適用範囲を示す単一値とし、同じ資料へ複数の`scope`行を記載しない。`all-workers`または個別workerを指定した場合は適用主体を、`all-threads`を指定した場合は全スレッドを対象とする共通資料であることを示す。`thread-scope`はworker向けルールの対象スレッドを絞る任意の単一値で、省略時は`all-threads`とする。両方がある場合は積集合として評価し、どちらか一方でも対象外なら適用しない。複数範囲を表す必要がある場合は、適用範囲を分けた別ルールへ分離し、和集合を推測しない。
