...
- サイドバー表示や独立チャット一覧に存在しないことを理由に不足と判定しない。親session配下のSubagent情報や結果資料をAPI取得できない場合は、未確認として取得範囲と代替証跡を記録する。代替証跡に矛盾がある場合、必須結果資料が欠落している場合、または既知の不一致・重複がある場合だけ停止する。
