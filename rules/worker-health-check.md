...
- `current-task.md`の論理責務、Subagent種別、subagentId、親sessionと実測結果が一致しない場合は、別workerとして自動的に置き換えず、不一致または重複として記録する。
...
- projectId、親session、Subagent ID、対象リポジトリ、Codex実行ディレクトリ、論理責務、指定モデル、推論レベルのいずれかを推測で補完しない。一致を確認できない場合は不一致またはアクセス不能として提示する。
